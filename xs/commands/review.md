...
- **Scope** — exactly what was reviewed: the whole repository, the staged/unstaged changes, or a comparison against `<base-branch>` (`git diff <base-branch>...HEAD`).
...
- **Findings** — the core of the report. List findings grouped by severity, highest first (or by area when that is clearer). Repeat one entry per finding. Each finding contains:
  ...
  - For change-based reviews (staged/unstaged or a branch comparison), mark a finding as a **regression** when it qualifies.
