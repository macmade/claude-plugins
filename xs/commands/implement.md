...
- Otherwise, find candidate plan documents in the repository and ask the user which one to use:
  - List plan files (names ending in `-plan.md` / `-plan.html`) via `git ls-files '*-plan.md' '*-plan.html'` plus untracked ones via `git ls-files -o --exclude-standard '*-plan.md' '*-plan.html'`. If none are found, fall back to listing all `*.md` / `*.html` files.
