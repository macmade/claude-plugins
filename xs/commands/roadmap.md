...
## 1. Determine whether to create a new roadmap or extend an existing one
...
- If no filename was provided, ask the user with `AskUserQuestion` as a **single-select** question whether to **create a new roadmap** or **extend an existing one**:
  - **Create a new roadmap** — continue with the title question below.
  - **Extend an existing roadmap** — find candidate roadmap files in the repository and ask the user which one to use, then treat the chosen file as an existing roadmap to extend exactly as in the argument case above (read it fully, detect its format, skip steps 2 and 5). Finding candidates needs a Git repository — if the current directory is not inside a Git working tree (`git rev-parse --is-inside-work-tree`), tell the user to pass the roadmap file as an argument instead.
    - List `*.md` / `*.html` files via `git ls-files '*.md' '*.html'` plus untracked ones via `git ls-files -o --exclude-standard '*.md' '*.html'`, then keep only those that actually look like roadmaps — inspect each candidate's contents rather than guessing from the name. Exclude standard repository boilerplate (README, LICENSE / COPYING, CODE_OF_CONDUCT, CONTRIBUTING, CHANGELOG, SECURITY, and similar conventional project files, regardless of extension or directory), plan files (names ending in `-plan.md` / `-plan.html`), source files that already have an associated plan (e.g. exclude `roadmap.md` when `roadmap-plan.md` or `roadmap-plan.html` exists alongside it), and files generated by the other commands (`code-review*`, `changes*`, and `release-notes*`).
    ...
    - Present them with `AskUserQuestion` as a **single-select** question. If no candidates exist, tell the user there is no existing roadmap to extend and offer to create a new one instead.

## 2. Choose the roadmap title

- Only when creating a new roadmap (when extending an existing one, reuse its title).
